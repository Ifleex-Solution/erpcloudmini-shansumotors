...
class Invoice extends MX_Controller
{
    ...
    public function save_sale()
    {
        ...
        $query = "
    INSERT INTO sale 
    (id,sale_id, date, details, type2, discount, total_discount_ammount, total_vat_amnt, grandTotal, total,customer_id,employee_id,payment_type,lastupdateddate,createddate,userid,incidenttype,already,branch,quotation_id) 
    VALUES 
    ...
    public function save_quotation()
    {
        ...
        $num = $this->number_generatorquotation($this->input->post('type2', TRUE));
        $lastupdate = date('Y-m-d H:i:s');

        $query = "
    INSERT INTO quotation 
    (id,sale_id, date, details, type2, discount, total_discount_ammount, total_vat_amnt, grandTotal, total,customer_id,employee_id,lastupdateddate,createddate,userid,incidenttype,already,branch) 
    VALUES 
    (0,AES_ENCRYPT('{$num}', '{$encryption_key}') , 
     '{$this->input->post('date', TRUE)}',
     '{$this->input->post('details', TRUE)}',  
    ...
    public function number_generatorquotation($type = null)
    {
        $encryption_key = Config::$encryption_key;
    ...
    {
        $postData = $this->input->post();
        $data = $this->invoice_model->quotation($postData, $this->input->post('type2'),$this->input->post('branchid'));
        echo json_encode($data);
    }
 ...
 po.payment_type, 
      ...
      WHERE pod.product = c.product
       ...
       ) AS avstock,AES_DECRYPT(sod.sale_id, '{$encryption_key}') as sales_order_no 
     ");
...
";
    ...
    public function quotation_print()
    {
        ...
        $sale = $this->quotation($this->input->post('id', TRUE));
        $saledetails = $this->quotationdetails($this->input->post('id', TRUE));
        $customer_info    = $this->customer_info($sale[0]['customer_id']);
        $company_info     = $this->service_model->company_info();
        $currency_details = $this->service_model->web_setting();
    ...
    public function quotationdetails($id = null)
    {
        $encryption_key = Config::$encryption_key;
    ...
    public function delete_quotation($id = null)
    {
        ...
        if ($productExists > 0) {
            ...
            echo '<script type="text/javascript">
            alert("Cannot delete this quotation detail because this quotation detail is linked to it or something went wrong");
